	...
	<div class="span12">
		...
		<?php if (!$twinesocial_appdata_json->features->hasRules) {?>
				...
				<div class="callout callout-info">
					...
					<a class="btn btn-danger" href="https://<?php echo TWINE_CUSTOMER_URL?>/account/choosePlan">Upgrade Now</a>
				</div>
				<!-- end callout -->
			<?php } ?>
				...
				<div class="twine-widget-content">
					...
					<?php foreach ($app->rules as $rule) { ?>
						<TR>
							...
							<TD><a target="_blank" class="btn btn-sm btn-default" href="https://<?php echo TWINE_CUSTOMER_URL?>/app/<?php echo $app->baseUrl?>/rules">Edit Rule</a></TD>
						</TR>
					<?php } ?>
					<?php } ?>
					</tbody>
			...
			<a id="btn-rules" class="btn btn-primary btn-sm" href="https://<?php echo TWINE_CUSTOMER_URL?>/app/<?php echo $twinesocial_appdata_json->apps[0]->baseUrl?>/rules/index">Add New Rule</a>
		</div>
	</div>
</div>
