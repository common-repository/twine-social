				...
				<div class="twine-widget-content">
					...
					<?php foreach ($campaign->connections as $connection) { ?>
						<TR>
							...
							<TD><a target="_blank" class="btn btn-sm btn-default" href="https:<?php echo TWINE_CUSTOMER_URL?>/app/<?php echo $campaign->baseUrl?>/connection">Edit Connection</a></TD>
						</TR>
					<?php } ?>
					<?php } ?>
					</tbody>
					</table>
					<a id="btn-rules" class="btn btn-primary btn-sm" href="https:<?php echo TWINE_CUSTOMER_URL?>/app/<?php echo $twinesocial_appdata_json->campaigns[0]->baseUrl?>/connection/index">Add New Social Connection</a>
				</div>
			</div>
		</div>
	</div>
